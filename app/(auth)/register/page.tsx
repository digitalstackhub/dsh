...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Account created!', description: 'Check your email for verification.', variant: 'success' })
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md">
          <div className="glass-card p-8 space-y-6">
              ...
              <div className="space-y-2">
                ...
                {password && (
                  <div className="flex gap-1 mt-2">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= passwordStrength(password) ? strengthColor(passwordStrength(password)) : 'bg-muted'}`} />
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase"><span className="bg-dark-800 px-2 text-muted-foreground">Or continue with</span></div>
            </div>
